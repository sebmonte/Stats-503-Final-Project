...
```{r}
#Since the intercept is just the mean, it is correct in its estimation of the general mean tendency of the data, and also since we are using bayes it is correctly placing some uncertaintiy around that estimate. Aside from having no predictors, this model is Gaussian so we are assuming that the distribution doesn't have skew. It also can't capture differences between groups as things stand.
```
...
```{r}
#Fitted() is capturing uncertainty in our parameters (the slope and intercept), but only predict() will add uncertainty in the noise (sigma), and therefore, it must always be wider.
```
...
```{r}
#pp_check does almost the same thing as what we did above: it draws from the posterior predictive distribution,
#for each draw, it simulates data with the same n as our initial data, and then plots the density
#The difference is that pp check automatically contrasted with with our real observed data using that dark line for y
```
...
```{r}
#The prior predictive check looks fairly reasonable in terms of its spread. Maybe it is concentrating too much mass on the mean. If it were predicting lengths of 500mm, that would mean the prior is way to weighted towards high bill length values and we had to adjust our initial expectations.
```
...
fit1.b$prior
fit2.b$prior
#the priors are identical, because brm uses a default prior if you do not use one. The results are nearly identical. However, had we set priors differently from the default, we could have obtained very different results.
```
...
```{r}
#It tells us that body mass *is* a good predictor, since when you add it, the model does better, specifically in terms of a high ELPD with better generalization.
```
