...
```{r}
# They give us a visual representation of the distribution of the data (where it is concentrated). For body mass and bill length, they are both concentrated in the middle generally although bill length is a little bimodal, with strong groupings at a higher and lower length point.
...
```{r}
fitted_vals <- fitted(fit1.ols)
resid_vals  <- residuals(fit1.ols)
...
summary(fit1.ols) 
summary(fit1.ols)$sigma #Residual standard error


#The residuals in summary of fit1.ols don't actually give us the mean directly but quartiles instead. The actual mean of residuals is always essentially zero because this is a result of calculating the deviations from the mean in both directions, which is what OLS is doing. 
#the residuals would be equal to yi - (b0 + b1xi) in the previous equation
```
...
```{r}
#Like in the regression output, we get a coefficient for the intercept and the predictor (body mass). However, the way we interpret these would be different (in the Bayesian case, the estimate would be the mean of the posterior). Unlike in the regression output, we also get an additional distributional parameter called sigma: this gives us a distribution over the noise as opposed to the residual standard error we got before. We do not get P values in the Bayesian case, just a credibility interval directly telling us what values are included
...
nd <- tibble(body_mass_g = seq(from = min(Gentoo$body_mass_g),
...
conditional_effects(fit2.brm)
```

```{r, warning = F}
bind_rows(
  tidy(fit2.ols, conf.int = TRUE) %>% select(term, estimate, contains("conf")),
  tidy(fit2.brm) %>% select(term, estimate, contains("conf")) %>% filter(term != "sd__Observation")
) %>% 
  mutate(method = rep(c("lm()", "brm()"), each = 2)) %>% 
...
```{r}
#
# Marginal Distributions: Shows the distribution of one parameter (diagonal)
#Joint distribution: shows how two parameters vary together (off diagonals)
...
```{r}
#For each draw, we are obtaining a set of parameters, so we end up getting a plausible distribution of parameters after doing 4000 draws. So our uncertainty is spread across them. For predictions, maybe we calculate regression one time for every single draw.
...
```{r}
#My best answer would be because sigma represents error and so it's always going to be above 0, making it likely to be skewed to the right.

#Notes from class: # One standard in Bayes: use data and find mean, and put that as mean of data, with a huge SD.
```
...
```{r}
# Since the mode, median, and mean are not the same, you might risk getting a skewed estimate (e.g., the mean might pull more to the right than you'd want). That's why median or mode might make more sense.
...
```{r}
#From the discussion in class, I'd say there is some default prior assumed in the Bayesian method that we are using. Maybe we could specify a different prior though if we wanted to plug in a custom one.
...
draws %>% 
  ...
  ggplot(aes(x = y_hat)) +
  geom_histogram(bins = 40) +
  labs(title = "Bayesians Posterior Distribution for Gentoo",
       x = expression(hat(italic(y))*'|'*italic(x)==3733.1)) +
  coord_cartesian(xlim = c(47, 51))
...
range(Gentoo$body_mass_g)

#Visualizing individual draws for Gentoo
draws %>% 
  select(.draw, beta0, beta1) %>% 
