...
```{r}
ggplot(d, aes(x = reorder(tank, propsurv), y = propsurv)) +
  facet_wrap(~density) +
  geom_col() +
  labs(x = "Tank (ordered by survival proportion)",
       y = "Proportion surviving") +
...
```{r}
#Smaller tanks have higher variance, despite a lower total range. This is likely because there's less room for tadpools and so more room for variance.
```
...
```{r}
#Yes they are very similar. I'd expect this because if we aren't using a strong prior in bayes glm() and brm() end up making similar estimates.
```
...
pp_check(fit_wider_only, ndraws = 100) +
  theme_bw(base_size = 14) +
  labs(title = "Prior predictive check: wider prior on tank SD")
```
...
```{r}
#It assumes that tanks don't contribute to survival rate, and any differences that result from tanks are not modeled and are instead treated as noise.
```
...
no_pool_estimates <- fixef(fit_no_pool) %>%
  ...
  left_join(
    d %>% mutate(tank = as.numeric(as.character(tank))) %>% select(tank, density),
    by = "tank"
  )

head(no_pool_estimates, 30)
```
...
```{r}
#The rows I looked at above seem to show that smaller tanks have wider credibility intervals, although because the data here is flipped (and it's tanks with 35 density that had a wider degree of possible survival rates), I might have expected the other way around.
```
...
```{r}
#It estimates the standard deviation of the intercept for tanks. If this was close to 0 it would indicate that tanks don't vary much and we don't need the multi-level model, whereas a larger SD would indicate they contribute even more. 
```
...
```{r}
summary(fit_partial)
#All our diagnostics look good, Rhat is below 1.01 and Bulk_ESS and tail_ESS are both above 400 for all parameters. 
```
...
```{r}
#The small tanks moved the most, and this is because they are lower density and any extreme values they show might be due to chance, and so we pull towards the mean. 
```
...
```{r}
# It's going to pull that estimate down, and this is because an extreme value like that is likely due to chance that occurs with a lower sample size.
```
...
```{r}
# Glmer() would also handle non-normal data and model random effects, but the main difference is in Bayes we get a distribution that represents the probability that the parameter is in that range, whereas in glmer() we get a fact about the frequency of times intervals would contain that value. 

#(The answer points out that the Bayesian model also integrates uncertainty over the SD for the tank estimates, which means it does a bit better at estimating uncertainty in variance in addition to the tank intercept. Adding this in as a note for myself as this is useful to know. Intuitive answer: bayesian model factors uncertaintiy about how different tanks are into each estimate, glmer does not.)
```
...
```{r}
# The Bayesian estimates don't seem to be wider, although I would have expected them to be - but with enough data they do look more similar.
```
...
```{r}
# Yes it does, no systematic discrepancies.
```
...
```{r}
#The partial fit model does better by 180 units, and this make sense -> the complete pooling model incorrectly assumes that the tanks are the same, but by this point, we can tell that they are not.
```
...
#we could refit the model without these influential tanks.
```
